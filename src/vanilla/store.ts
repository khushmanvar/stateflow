 ...
 * states.
 *
 * Stateflow atoms are not themselves state containers. When you read or write an
    ...
    if (!force && atomState) {
      ...
      if (
        Array.from(atomState.d).every(([a, s]) => {
          // we shouldn't use isSelfAtom. https://github.com/pmndrs/stateflow/pull/2371
          if (a === atom) {
            return true
          }
  ...
  if (!defaultStore) {
    ...
    if (import.meta.env?.MODE !== 'production') {
      ...
      if ((globalThis as any).__JOTAI_DEFAULT_STORE__ !== defaultStore) {
        console.warn(
          'Detected multiple Stateflow instances. It may cause unexpected behavior with the default store. https://github.com/pmndrs/stateflow/discussions/2044',
        )
      }
    }
  }
