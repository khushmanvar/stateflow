...
export default function reactRefreshPlugin(
  ...
  return {
    ...
    visitor: {
      Program: {
        exit(path) {
          const stateflowAtomCache = templateBuilder(`
          ...
          path.unshiftContainer('body', stateflowAtomCache)
        },
      },
      ExportDefaultDeclaration(nodePath, state) {
        ...
        if (
          ...
          const buildExport = templateBuilder(
            `export default globalThis.stateflowAtomCache.get(%%atomKey%%, %%atom%%)`,
          )
      ...
      VariableDeclarator(nodePath, state) {
        if (
          ...
          const buildAtomDeclaration = templateBuilder(
            `const %%atomIdentifier%% = globalThis.stateflowAtomCache.get(%%atomKey%%, %%atom%%)`,
          )
