...
[stateflow-effect](https://github.com/stateflowjs/stateflow-effect) is a utility package for reactive side effects.

## install

```
npm i stateflow-effect
```

## atomEffect

`atomEffect` is a utility function for declaring side effects and synchronizing atoms in Stateflow. It is useful for observing and reacting to state changes.
...
**effectFn** (required): A function for listening to state updates with `get` and writing state updates with `set`. The `effectFn` is useful for creating side effects that interact with other Stateflow atoms. You can cleanup these side effects by returning a cleanup function.
...
```js
import { atomEffect } from 'stateflow-effect'
...
After defining an effect using `atomEffect`, it can be integrated within another atom's read function or passed to Stateflow hooks.
...
- **Executes In The Next Microtask:**
  `effectFn` runs in the next available microtask, after all Stateflow synchronous read evaluations have completed.
...
For synchronizing component props and state with a Stateflow atom, you should use the useEffect hook.

#### Global Side Effects

For setting up global side-effects, deciding between useEffect and atomEffect comes down to developer preference.
Whether you prefer to build this logic directly into the component or build this logic into the Stateflow state model depends on what mental model you adopt.
