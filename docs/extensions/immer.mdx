...
You have to install `immer` and `stateflow-immer` to use this feature.

```
npm i immer stateflow-immer
```
...
```jsx
import { useAtom, atom } from 'stateflow'
import { withImmer } from 'stateflow-immer'
...
```jsx
import { useAtom } from 'stateflow'
import { useImmerAtom } from 'stateflow-immer'
