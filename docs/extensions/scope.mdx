...
There are a few libraries to extend Stateflow's usage in React.

## `stateflow-scope`

While Stateflow's Provider allows to scope Stateflow's store under a subtree,
we can't use the store above the tree within the subtree.

A workaround is to use `store` option in useAtom and other hooks.
To ease such use cases, `stateflow-scope` provides a
...
import { ScopeProvider } from 'stateflow-scope'
...
Both Stateflow's Provider and `stateflow-scope`'s scoped provider
are still using global contexts.

If you are developing a library that depends on Stateflow and
the library user may use Stateflow separately in their apps,
they can share the same context. This can be troublesome
because they point to unexpected Stateflow stores.

To avoid conflicting the contexts,
a utility function called `createIsolation` is exported from `stateflow-scope`.

Check out the example in the repo: https://github.com/stateflowjs/stateflow-scope/tree/main/examples/01_isolation

## `bunshi` (formerly `stateflow-molecules`)

Stateflow atoms provide a basic solution to optimize re-renders.
