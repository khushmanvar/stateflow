...
You can use Stateflow with [Relay](https://relay.dev).

### Install

You have to install `stateflow-relay` and `relay-runtime`.

```
npm i stateflow-relay relay-runtime
```
...
import { Provider, useAtom } from 'stateflow'
import { useHydrateAtoms } from 'stateflow/utils'
import { environmentAtom, atomWithQuery } from 'stateflow-relay'
