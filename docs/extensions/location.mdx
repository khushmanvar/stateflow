...
You have to install `stateflow-location` to use this feature.

```
npm i stateflow-location
```
...
import { atomWithLocation } from 'stateflow-location'
...
**setHash** (optional): `replaceState` or a custom function that describes how hash gets updated on the side of the browser. Defaults to pushing a new entry to the history via `window.location.hash = searchParams` ([stateflow-location#4](https://github.com/stateflowjs/stateflow-location/pull/4))

### Examples

```jsx
import { useAtom } from 'stateflow'
import { atomWithHash } from 'stateflow-location'
