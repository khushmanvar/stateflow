...
You can use Stateflow with [tRPC](https://trpc.io).

### Install

You have to install `stateflow-trpc`, `@trpc/client` and `@trpc/server` to use the extension.

```
npm i stateflow-trpc @trpc/client @trpc/server
```

### Usage

```ts
import { createTRPCStateflow } from 'stateflow-trpc'

const trpc = createTRPCStateflow<MyRouter>({
  links: [
    httpLink({
      url: myUrl,
    }),
...
const trpc = createTRPCStateflow<PokemonRouter>({
  links: [
    httpLink({
      url: trpcPokemonUrl,
    }),
