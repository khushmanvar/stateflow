...
[stateflow-urql](https://github.com/stateflowjs/stateflow-urql) is a Stateflow extension library for URQL. It offers a cohesive interface that incorporates all of URQL's GraphQL features, allowing you to leverage these functionalities alongside your existing Stateflow state.

### Install

You have to install `stateflow-urql`, `@urql/core` and `wonka` to use the extension.

```
npm i stateflow-urql @urql/core wonka
```
...
Usage of `import { loadable } from "stateflow/utils"` is preferred instead as proven more stable. However is you still want that
here is how you do it:

```tsx
import { suspenseAtom } from 'stateflow-urql'
...
import { useAtom } from 'stateflow'
import type { AtomWithQuery } from 'stateflow-urql'
...
>(
  ...
  if (opResult.error && opResult.stale) {
    use(
      ...
      // (in case of Stateflow URQL bindings only).
...
import { clientAtom } from 'stateflow-urql'
