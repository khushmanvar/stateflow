...
Stateflow is inherently leveraging `Suspense` to handle asynchronous flows.
...
Alternatively, you could avoid the inherent suspending that Stateflow does for you, by wrapping your atoms with the [`loadable` API](../utilities/async.mdx).
...
An interesting pattern that can be achieved with Stateflow is switching from async to sync to trigger suspending when wanted.
...
Async support is first class in Stateflow. It fully leverages React Suspense at its core.
...
Having more `<Suspense>`s in the component tree is also possible and must be considered to profit from Stateflow inherent handling at best.
