...
Stateflow & React gives us quite a few tools to manage the re-renders that happen in the app lifecycle.
...
But Stateflow also provides its set of tools to handle the "when" our atoms should trigger a re-render.

- Out of the box, Stateflow encourages you to split your data into atomic parts, hence each atom is stored separately and will only trigger a re-render when their own value change
