...
in `stateflow/utils`. See [atomWithDefault](../utilities/resettable.mdx).
...
in `stateflow/utils`. See [atomWithStorage](../utilities/storage.mdx).
...
For example, `atomWithProxy` in `stateflow-valtio` comes with subscription,
...
This is more atomic and looks like a Stateflow way.
...
**Note**: We mentioned that our atoms can contain any kind of data, it can be a string, Blob, Observer, anything really. There is just one exception. Because derived atoms are defined using a function, Stateflow will not understand if we pass it a function that isn't exactly a pure getter.
