---
title: Next.js
description: How to use Stateflow with Next.js
nav: 7.03
...
It's possible to sync Stateflow with the router. You can achieve this with `atomWithHash`:
...
By default, Stateflow uses an implicit global store to keep track of atom values. This is what is referred to as "provider-less" mode. This becomes an issue in SSR scenario because this global store is kept alive and is shared between multiple requests, which can lead to bugs and security risks.

To limit the lifetime of the store to the scope of one request, you need to use a [Provider](../core/provider.mdx) at the root of your app (or a subtree if you're using Stateflow only for a part of your application).
...
Stateflow provides SWC plugins for better DX while developing with Next.js. [Find more info in the SWC section.](../tools/swc.mdx)
...
```bash
npx create-next-app --example with-stateflow with-stateflow-app
```

Here's a [link](https://github.com/vercel/next.js/tree/canary/examples/with-stateflow).
