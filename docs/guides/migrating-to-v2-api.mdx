...
RFC: https://github.com/pmndrs/stateflow/discussions/1514

Stateflow v1 is released at June 2022, and there has been various feedbacks.
React also proposes first-class support for promises.
Stateflow v2 will have a new API.
...
Stateflow comes with vanilla (non-React) functions
and React functions separately.
They are provided from alternate entry points like `stateflow/vanilla`.

#### Store API

Stateflow exposes store interface so that you can directly manipulate atom values.

```js
import { createStore } from 'stateflow' // or from 'stateflow/vanilla'
...
This applies to some other utils like `atomsWithQuery` from `stateflow-tanstack-query`.
...
- https://github.com/pmndrs/stateflow/discussions/1615
- https://github.com/stateflowjs/stateflow-tanstack-query/issues/21
- https://github.com/pmndrs/stateflow/discussions/1751

### Some other changes

#### Utils

- `atomWithStorage` util's `delayInit` is removed as being default. Also it will always render `initialValue` on first render, and the stored value, if any, on subsequent renders. The new behavior differs from v1. See https://github.com/pmndrs/stateflow/discussions/1737 for more information.
...
- `stateflow/vanilla`
- `stateflow/vanilla/utils`
- `stateflow/react`
- `stateflow/react/utils`
...
import { atom } from 'stateflow' // is same as 'stateflow/vanilla'
import { useAtom } from 'stateflow' // is same as 'stateflow/react'
```

Note: If you are not using ESM, you want to prefer using `stateflow/vanilla` etc. instead of `stateflow`, for better tree shaking.
