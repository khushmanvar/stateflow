...
> `focusAtom` creates a new atom, based on the focus that you pass to it. [stateflow-optics](../extensions/optics.mdx#focus-atom)
...
> The `splitAtom` utility is useful when you want to get an atom for each element in a list. [stateflow/utils](../utilities/split.mdx)
...
> This function creates a derived atom whose value is a function of the original atom's value. [stateflow/utils](../utilities/select.mdx)
...
import { selectAtom, splitAtom } from 'stateflow/utils'
