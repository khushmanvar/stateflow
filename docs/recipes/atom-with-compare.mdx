...
Note: Stateflow uses `Object.is` internally to compare values when changes occur. If `areEqual(a, b)` returns false, but `Object.is(a, b)` returns true, Stateflow will not trigger an update.

```ts
import { atomWithReducer } from 'stateflow/utils'
