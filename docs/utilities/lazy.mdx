...
Ref: https://github.com/pmndrs/stateflow/pull/2465
...
```jsx
import { atomWithLazy } from 'stateflow/utils'
...
Since each store is its separate universe, the initial value will be recreated exactly once per store (unless using something like `stateflow-scope`, which fractures a store into smaller universes).
