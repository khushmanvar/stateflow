---
title: Devtools
description: This doc describes Devtools for Stateflow.
nav: 5.03
...
Install `stateflow-devtools` to your project to get started.

```sh
npm i stateflow-devtools
```
...
- Feedback welcome, please file issues or ask questions on the [Stateflow DevTools GitHub Repo](https://github.com/stateflowjs/stateflow-devtools/discussions)
...
- Migration guides
  - [Migrate from `@emotion/react`](https://github.com/stateflowjs/stateflow-devtools?tab=readme-ov-file#migrate-%C6%92rom-emotionreact-to-native-css)

### UI DevTools

Enhance your development experience with the UI based Stateflow DevTool.

#### Babel plugin setup - (_Optional but highly recommended_)

Use Stateflow babel plugins for optimal debugging experience. Find the complete guide on the [babel](../tools/babel) page and/or [swc](../tools/swc) page.
...
```ts
{
  "presets": [
    ...
    // - stateflow/babel/plugin-react-refresh to enable hot reaload for atoms
    // - stateflow/babel/plugin-debug-label to automatically adds debug labels to atoms
    'stateflow/babel/preset'
  ]
}
```
...
const nextConfig = {
  ...
  transpilePackages: ['stateflow-devtools'],
}
...
type DevToolsProps = {
  ...
  // Custom nonce to allowlist stateflow-devtools specific inline styles via CSP
  nonce?: string
  options?: {
...
```tsx
import { DevTools } from 'stateflow-devtools'
import 'stateflow-devtools/styles.css'
...
```jsx
import { useAtomsDebugValue } from 'stateflow-devtools/utils'
...
```ts
import { useAtomDevtools } from 'stateflow-devtools/utils'
...
import { useAtomsSnapshot } from 'stateflow-devtools/utils'
...
`useGotoAtomsSnapshot` will update the current Stateflow state to match the passed snapshot.
...
This hook returns a callback, which takes a `snapshot` from the `useAtomsSnapshot` hook and will update the Stateflow state. It accepts a `store` parameter.
This hook is primarily meant for debugging and devtools use cases.
...
```jsx
import { Provider } from 'stateflow'
import { useAtomsSnapshot, useGotoAtomsSnapshot } from 'stateflow-devtools/utils'
