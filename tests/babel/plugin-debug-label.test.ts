...
import plugin from 'stateflow/babel/plugin-debug-label'
...
it('Should handle a atom from a default export', () => {
  expect(transform(`const countAtom = stateflow.atom(0);`)).toMatchInlineSnapshot(`
    "const countAtom = stateflow.atom(0);
    countAtom.debugLabel = "countAtom";"
  `)
})
