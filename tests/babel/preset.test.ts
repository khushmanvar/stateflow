...
import preset from 'stateflow/babel/preset'
...
it('Should add a debugLabel and cache to an atom', () => {
  expect(transform(`const countAtom = atom(0);`, '/src/atoms.ts'))
    .toMatchInlineSnapshot(`
      ...
      const countAtom = globalThis.stateflowAtomCache.get("/src/atoms.ts/countAtom", atom(0));
...
it('Should add a debugLabel and cache to multiple atoms', () => {
  ...
  const doubleAtom = atom((get) => get(countAtom) * 2);`,
    ...
    const countAtom = globalThis.stateflowAtomCache.get("/src/atoms.ts/countAtom", atom(0));
    countAtom.debugLabel = "countAtom";
    const doubleAtom = globalThis.stateflowAtomCache.get("/src/atoms.ts/doubleAtom", atom(get => get(countAtom) * 2));
...
it('Should add a cache and debugLabel for a default exported atom', () => {
  expect(transform(`export default atom(0);`, '/src/atoms/index.ts'))
    .toMatchInlineSnapshot(`
      ...
      const atoms = globalThis.stateflowAtomCache.get("/src/atoms/index.ts/atoms", atom(0));
...
it('Should add a cache and debugLabel for mixed exports of atoms', () => {
  ...
  `,
    ...
    export const countAtom = globalThis.stateflowAtomCache.get("/src/atoms/index.ts/countAtom", atom(0));
    countAtom.debugLabel = "countAtom";
    const atoms = globalThis.stateflowAtomCache.get("/src/atoms/index.ts/atoms", atom(get => get(countAtom) * 2));
    atoms.debugLabel = "atoms";
    export default atoms;"
  `)
})
...
it('Should handle custom atom names', () => {
  expect(
    ...
    const mySpecialThing = globalThis.stateflowAtomCache.get("/src/atoms.ts/mySpecialThing", myCustomAtom(0));
