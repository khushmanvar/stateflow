...
import plugin from 'stateflow/babel/plugin-react-refresh'
...
it('Should add a cache for a single atom', () => {
  expect(transform(`const countAtom = atom(0);`, '/src/atoms/index.ts'))
    .toMatchInlineSnapshot(`
      "globalThis.stateflowAtomCache = globalThis.stateflowAtomCache || {
        cache: new Map(),
        get(name, inst) {
      ...
      const countAtom = globalThis.stateflowAtomCache.get("/src/atoms/index.ts/countAtom", atom(0));"
    `)
})
...
it('Should add a cache for multiple atoms', () => {
  ...
  `,
    ...
    const countAtom = globalThis.stateflowAtomCache.get("/src/atoms/index.ts/countAtom", atom(0));
    const doubleAtom = globalThis.stateflowAtomCache.get("/src/atoms/index.ts/doubleAtom", atom(get => get(countAtom) * 2));"
  `)
})
...
it('Should add a cache for multiple exported atoms', () => {
  ...
  `,
    ...
    export const countAtom = globalThis.stateflowAtomCache.get("/src/atoms/index.ts/countAtom", atom(0));
    export const doubleAtom = globalThis.stateflowAtomCache.get("/src/atoms/index.ts/doubleAtom", atom(get => get(countAtom) * 2));"
  `)
})
...
it('Should add a cache for a default exported atom', () => {
  expect(transform(`export default atom(0);`, '/src/atoms/index.ts'))
    .toMatchInlineSnapshot(`
      ...
      export default globalThis.stateflowAtomCache.get("/src/atoms/index.ts/defaultExport", atom(0));"
    `)
})
...
it('Should add a cache for mixed exports of atoms', () => {
  ...
  `,
    ...
    export const countAtom = globalThis.stateflowAtomCache.get("/src/atoms/index.ts/countAtom", atom(0));
    export default globalThis.stateflowAtomCache.get("/src/atoms/index.ts/defaultExport", atom(get => get(countAtom) * 2));"
  `)
})
...
it('Should handle custom atom names', () => {
  expect(
    ...
    const mySpecialThing = globalThis.stateflowAtomCache.get("/src/atoms/index.ts/mySpecialThing", myCustomAtom(0));"
  `)
})
